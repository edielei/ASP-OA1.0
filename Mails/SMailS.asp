...
bt=Easp.Get("bt:s")
sj=Easp.Get("sj:s")
if Easp.IsN(bt) and Easp.IsN(sj) then Easp.RR "SMail.asp"
%><!DOCTYPE html>
<html lang="cn">
<head>
...
<%Dim rs,t,i
if Easp.IsN(sj) then
Set rs=db.GPR("0:15",Array("SMail:id,bt,fjr,isread,sj","isdel = false and sjr='"&Easp.Cookie("uinfo>uname:s")&"' and bt like '%"&bt&"%'","id desc"))
ElseIf Easp.IsN(bt) then
Set rs=db.GPR("0:15",Array("SMail:id,bt,fjr,isread,sj","isdel = false and sjr='"&Easp.Cookie("uinfo>uname:s")&"' and format(sj,'yyyy-mm-dd') = '"&Easp.DateTime(sj,"yyyy-mm-dd")&"'","id desc"))
Else
Set rs=db.GPR("0:15",Array("SMail:id,bt,fjr,isread,sj","isdel = false and sjr='"&Easp.Cookie("uinfo>uname:s")&"' and bt like '%"&bt&"%' and format(sj,'yyyy-mm-dd') = '"&Easp.DateTime(sj,"yyyy-mm-dd")&"'","id desc"))
End if
if Easp.IsN(rs) then
...
db.C(rs)
db.SetPager "default", "总共{recordcount}条记录  每页{pagesize}条 {pageindex}/{pagecount}页 {first}{prev}{liststart}{list}{listend}{next}{last}转到 {jump} 页",Array("listlong:5","link:SMailS.asp?page=*&bt="&Server.URLEncode(Replace(bt,"''","'"))&"&sj="+Server.URLEncode(sj))
