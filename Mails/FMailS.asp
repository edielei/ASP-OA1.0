...
bt=Easp.Get("bt:s")
if Easp.IsN(bt) then Easp.RR "FMail.asp"
%><!DOCTYPE html>
<html lang="cn">
<head>
...
<body>
<div id="sousuo">标题：<input id="bt" class="input" style="width:120px;" value="<%=Replace(bt,"''","'")%>" /> <input id="tj" type="submit" value="搜索" class="btnss" /></div>
...
<%
Dim rs,t,i
Set rs=db.GPR("0:15",Array("FMail:id,bt,unames","fjr='"&Easp.Cookie("uinfo>uname:s")&"' and bt like '%"&bt&"%'","id desc"))
if Easp.IsN(rs) then
Easp.W "<tr><td class=""td"" align=""center"" colspan=""5"">找不到你要查找的邮件！</td></tr></table>"
Else
t=rs.GetRows(rs.PageSize)
db.C(rs)
db.SetPager "default", "总共{recordcount}条记录  每页{pagesize}条 {pageindex}/{pagecount}页 {first}{prev}{liststart}{list}{listend}{next}{last}转到 {jump} 页",Array("listlong:5","link:FMailS.asp?page=*&bt="&Server.URLEncode(Replace(bt,"''","'")))
