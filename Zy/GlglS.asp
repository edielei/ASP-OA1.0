...
<body>
<div id="sousuo"><form action="GlglS.asp" method="get">标题：<input id="bt" name="bt" class="input" style="width:130px;" value="<%=Replace(bt,"''","'")%>" />&nbsp;或&nbsp;时间：<input id="sj" name="sj" class="srk" style="width:80px;" onfocus="WdatePicker()" value="<%=sj%>" /> <input id="tj" type="submit" class="btn" value="搜索" /></form></div>
...
Dim rs,t,i
if Easp.IsN(sj) then
Set rs=Easp.db.GPR("0:15",Array("Zy:files,bt,sj,uname,ms,id","bt like '%"&bt&"%'","id desc"))
Elseif Easp.IsN(bt) then
Set rs=Easp.db.GPR("0:15",Array("Zy:files,bt,sj,uname,ms,id","format(sj,'yyyy-mm-dd')='"&sj&"'","id desc"))
Else
Set rs=Easp.db.GPR("0:15",Array("Zy:files,bt,sj,uname,ms,id","bt like '%"&bt&"%' and format(sj,'yyyy-mm-dd')='"&sj&"'","id desc"))
End if
if Easp.IsN(rs) then
...
Easp.db.C(rs)
Easp.db.SetPager "default", "总共{recordcount}条记录  每页{pagesize}条 {pageindex}/{pagecount}页 {first}{prev}{liststart}{list}{listend}{next}{last}转到 {jump} 页",Array("listlong:5","link:GlglS.asp?page=*&bt="&Server.URLEncode(Replace(bt,"''","'"))&"&sj="&sj)
