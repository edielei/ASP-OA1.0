<!--#include file="../Inc/GzjhCs.asp"--><%Dim d
d=Easp.Get("date:d")
%><!DOCTYPE html>
<html lang="cn">
<head>
...
<body>
<div id="sousuo"><span style="float:left;"><input class="btnss" type="button" value="新建" onClick="add()" /> <input type="button" value="类别管理" class="btnss" style="width:65px;" onClick="location='GzjhTypes.asp'" /></span>类别：<%Easp.Ext("ed").JhTypes()%>　标题：<input id="bt" class="input" style="width:120px;" /> <input id="tj" type="submit" value="搜索" class="btnss" /></div>
<table border="0" cellpadding="0" cellspacing="0" class="table">
	...
	Dim rs,t,i
	if Easp.Has(d) then
	Set rs=db.GPR("0:15",Array("V_Gzjh:id,bt,kssj,jssj,types,typesid","uname='"&Easp.Cookie("uinfo>uname:s")&"' and format(kssj,'yyyy-mm-dd')='"&Easp.DateTime(d,"yyyy-mm-dd")&"'","id desc"))
	Else
	Set rs=db.GPR("0:15",Array("V_Gzjh:id,bt,kssj,jssj,types,typesid","uname='"&Easp.Cookie("uinfo>uname:s")&"'","id desc"))
	End if
	if Easp.IsN(rs) then
	Easp.W "<tr><td class=""td"" align=""center"" colspan=""6"">暂无工作计划</td></tr></table>"
	Else
    ...
    <tr>
    	...
    	<td class="td"><a href="GzjhT.asp?id=<%=t(5,i)%>"><%Easp.WH t(4,i)%></a></td>
