...
Dim id,r
id=Easp.Get("id:n:0")
r=Easp.db.RT("users","id="&id,"uname,zname,qx,yjlx,isgg,iszy,istxl,iskq,yjlx,ishr,isbbs,istp,issys,iswl")
%><!DOCTYPE html>
<html lang="cn">
<head>
...
$(function(){
	var tr3=$('tr:eq(3)');
	$(':radio[value="<%=r(3)%>"]').attr('checked','checked');
	$('#qx').change(function(){
		var a=$(this).val();
		switch(a){
	...
	$('#isgg').attr('checked',<%=Lcase(r(4))%>);
	$('#iszy').attr('checked',<%=Lcase(r(5))%>);
	$('#istxl').attr('checked',<%=Lcase(r(6))%>);
	$('#iskq').attr('checked',<%=Lcase(r(7))%>);
	$('#yjlx').attr('checked',<%=Lcase(r(8))%>);
	$('#ishr').attr('checked',<%=Lcase(r(9))%>);
	$('#isbbs').attr('checked',<%=Lcase(r(10))%>);
	$('#istp').attr('checked',<%=Lcase(r(11))%>);
	$('#issys').attr('checked',<%=Lcase(r(12))%>);
	$('#iswl').attr('checked',<%=Lcase(r(13))%>);
	var s=$('#tj');
	s.click(function(){
		...
		var iswl=$('#iswl').attr('checked');	//获取网络会议管理权限);
		$.post('QxEditC.asp','id=<%=id%>&qx='+qx+'&isgg='+isgg+'&iszy='+iszy+'&istxl='+istxl+'&iskq='+iskq+'&yjlx='+yjlx+'&ishr='+ishr+'&isbbs='+isbbs+'&istp='+istp+'&issys='+issys+'&iswl='+iswl,function(a){
			switch(a){
				case 'OK':
					art.dialog.open.api.hide();
					art.dialog.through({icon:'succeed',content:'修改权限分配成功！',resize:false,ok:function(){
