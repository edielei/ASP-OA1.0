...
<body>
<div id="sousuo">所属版块：<%Easp.Ext("ed").BkTypes()%> 标题：<input id="bt" class="input" style="width:120px;" value="<%=Replace(bt,"''","'")%>" /> <input type="submit" id="tj" value="搜索" class="btnss" /></div>
<table border="0" cellpadding="0" cellspacing="0" class="table">
	...
	Dim rs,t,i
	if Easp.IsN(id) then
	Set rs=db.GPR("0:15",Array("MyTz:id,bt,bk,sj,hfcs,cs,bkid,uname","bt like '%"&bt&"%'","id desc"))
	Else
	Set rs=db.GPR("0:15",Array("MyTz:id,bt,bk,sj,hfcs,cs,bkid,uname","bkid="&id&" and bt like '%"&bt&"%'","id desc"))
	End If
	if Easp.IsN(rs) then
	Easp.W "<tr><td class=""td"" align=""center"" colspan=""6"">找不到你要查找的帖子！</td></tr></table>"
	Else
