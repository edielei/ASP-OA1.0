...
Dim bsc
bsc=Easp.Get("bsc:s")
if Easp.IsN(bsc) then Easp.RR "Bsc.asp"
%><!DOCTYPE html>
<html lang="cn">
<head>
...
Next
End if%></select>　姓名：<input id="zname" class="input" style="width:68px;" maxlength="12" value="" />&nbsp;<input id="tj" type="submit" value="搜索" class="btnss" /></div>
...
Set rs=db.GPR("0:15",Array("Bsc:id,zname,mc,dz,shouji,zj,email","mc='"&bsc&"'","id desc"))
if Easp.IsN(rs) then
Easp.W "<tr><td class=""td"" align=""center"" colspan=""6"">暂无记录！</td></tr></table>"
Else
t=rs.GetRows(rs.PageSize)
db.C(rs)
db.SetPager "default", "总共{recordcount}条记录  每页{pagesize}条 {pageindex}/{pagecount}页 {first}{prev}{liststart}{list}{listend}{next}{last}转到 {jump} 页",Array("listlong:5","link:BscT.asp?bsc="&bsc&"&page=*")
