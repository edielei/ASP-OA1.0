...
if Easp.IsN(bm) or Easp.IsN(zname) then Easp.RR "Nb.asp"
%><!DOCTYPE html>
<html lang="cn">
<head>
...
<%
Dim rs,t,i
Set rs=Easp.db.GPR("0:15",Array("V_Txl:zname,bt,zx,fj,shouji,email","bt='"&bm&"' and zname like '%"&zname&"%'","utime asc"))
if Easp.IsN(rs) then
Easp.W "<tr><td class=""td"" colspan=""6"" align=""center"">找不到你要查找的记录！</td></tr>"
Else
t=rs.GetRows(rs.PageSize)
Easp.db.C(rs)
Easp.db.SetPager "default", "总共{recordcount}条记录  每页{pagesize}条 {pageindex}/{pagecount}页 {first}{prev}{liststart}{list}{listend}{next}{last}转到 {jump} 页",Array("listlong:5","link:NbSx.asp?page=*&bm="&Server.URLEncode(bm)&"&zname="&Server.URLEncode(zname))
