...
if Easp.IsN(zname) then Easp.RR "BscGl.asp"
%><!DOCTYPE html>
<html lang="cn">
<head>
...
$(function(){ 
	...
	s.click(function(){
		...
		location.href="BscGlN.asp?zname="+zname;
	});
	$('#bsc').change(function(){
...
Set rs=db.GPR("0:15",Array("Bsc:id,zname,mc,dz,shouji,zj,email","zname like '%"&zname&"%'","id desc"))
if Easp.IsN(rs) then
Easp.W "<tr><td class=""td"" align=""center"" colspan=""7"">找不到你要查找的记录！</td></tr></table>"
Else
t=rs.GetRows(rs.PageSize)
db.C(rs)
db.SetPager "default", "总共{recordcount}条记录  每页{pagesize}条 {pageindex}/{pagecount}页 {first}{prev}{liststart}{list}{listend}{next}{last}转到 {jump} 页",Array("listlong:5","link:BscGlN.asp?page=*&zname="&Server.URLEncode(zname))
