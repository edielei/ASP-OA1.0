...
$(function(){
	...
	s.click(function(){
		...
		$.post('CcdjC.asp', {'kssj':kssj.val(),'jssj':jssj.val(),'nr':nr}, function(a){
			switch(a){
				...
				art.dialog.through({
					...
					time:3,
					close:function(){
						window.parent['mkj'].location='/KaoQin/SeeCc.asp';
						art.dialog.close();
					}
				});
				break;
...
<body>
<table>
	<tr><td nowrap="nowrap">开始时间：</td><td><input id="kssj" class="input" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',maxDate:'#F{$dp.$D(\'jssj\',{m:-1})}'})" />&nbsp;&nbsp;&nbsp;&nbsp;结束时间：<input id="jssj" class="input" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'kssj\',{m:+1})}'})" /></td></tr>
	<tr><td nowrap="nowrap">出差原因：</td><td><textarea id="nr" class="input"></textarea></td></tr>
